...
public class Dashboard extends BaseActivity {
    ...
    public void updateOverviewUI(String hospitalId, MaterialCardView appointmentsSection, MaterialCardView admittedSection, MaterialCardView operationsSection) {
        ...
        subListeners.add(db.collection("hospitals").document(hospitalId).collection("treatments")
                ...
                .whereGreaterThanOrEqualTo("start", startOfToday)
                .addSnapshotListener((s, e) -> {
                    if (s != null && tvAppointmentCount != null) tvAppointmentCount.setText(String.valueOf(s.size()));
                }));

        // 2. Currently Admitted Count
        // Ensure R.id.admitted_count exists inside the admittedSection layout
        ...
        subListeners.add(db.collection("hospitals").document(hospitalId).collection("treatments")
                .whereEqualTo("type", TreatmentType.OPERATION.name())
                .whereGreaterThanOrEqualTo("start", startOfToday)
                .addSnapshotListener((s, e) -> {
                    if (s != null && tvOperationsCount != null) tvOperationsCount.setText(String.valueOf(s.size()));
                }));
    }
